...
GetSessionsCommand::GetSessionsCommand(
    const QString &userId, const QString &taskId, const QString &baseUrl, QObject *parent)
    : IApiCommand(parent)
    , mUserId(userId)
    , mTaskId(taskId)
    , mBaseUrl(baseUrl)
...
void GetSessionsCommand::execute()
{
	...
	connect(
	    mApiClient.get(),
	    &OpenAPI::OAIPomodoroServiceApi::pomodoroServiceGetSessionsSignal,
	    this,
	    &GetSessionsCommand::onSessionsRetrieved);
	connect(
	    mApiClient.get(),
	    &OpenAPI::OAIPomodoroServiceApi::pomodoroServiceGetSessionsSignalError,
	    this,
	    &GetSessionsCommand::onSessionsError);
...
void GetSessionsCommand::onSessionsError(
