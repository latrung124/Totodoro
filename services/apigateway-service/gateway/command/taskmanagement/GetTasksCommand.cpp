...
GetTasksCommand::GetTasksCommand(
    const QString &userId, const QString &groupId, const QString &baseUrl, QObject *parent)
    : IApiCommand(parent)
    , mUserId(userId)
    , mGroupId(groupId)
    , mBaseUrl(baseUrl)
...
void GetTasksCommand::execute()
{
	...
	connect(
	    mApiClient.get(),
	    &OpenAPI::OAITaskManagementServiceApi::taskManagementServiceGetTasksSignal,
	    this,
	    &GetTasksCommand::onTasksRetrieved);
	connect(
	    mApiClient.get(),
	    &OpenAPI::OAITaskManagementServiceApi::taskManagementServiceGetTasksSignalError,
	    this,
	    &GetTasksCommand::onTasksError);
...
void GetTasksCommand::onTasksError(
