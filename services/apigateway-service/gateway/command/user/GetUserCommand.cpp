...
GetUserCommand::GetUserCommand(const QString &userId, const QString &baseUrl, QObject *parent)
    : IApiCommand(parent)
    , mUserId(userId)
    , mBaseUrl(baseUrl)
...
void GetUserCommand::execute()
{
	...
	connect(
	    mApiClient.get(),
	    &OpenAPI::OAIUserServiceApi::userServiceGetUserSignal,
	    this,
	    &GetUserCommand::onUserRetrieved);
	connect(
	    mApiClient.get(),
	    &OpenAPI::OAIUserServiceApi::userServiceGetUserSignalError,
	    this,
	    &GetUserCommand::onUserError);
...
void GetUserCommand::onUserError(
    const OAIUser_serviceGetUserResponse &response,
