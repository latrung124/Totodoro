 ...
 * @author trung.la
 * @date 09-18-2025
 * @brief Implementation of GetSettingsCommand class that handles fetching user settings via the
 * OpenAPI client
 */
...
GetSettingsCommand::GetSettingsCommand(
    const QString &userId, const QString &baseUrl, QObject *parent)
    : IApiCommand(parent)
    , mUserId(userId)
    , mBaseUrl(baseUrl)
...
void GetSettingsCommand::execute()
{
	...
	connect(
	    mApiClient.get(),
	    &OpenAPI::OAIUserServiceApi::userServiceGetSettingsSignal,
	    this,
	    &GetSettingsCommand::onSettingsRetrieved);
	connect(
	    mApiClient.get(),
	    &OpenAPI::OAIUserServiceApi::userServiceGetSettingsSignalError,
	    this,
	    &GetSettingsCommand::onSettingsError);
...
void GetSettingsCommand::onSettingsError(
    const OAIUser_serviceGetSettingsResponse &response,
