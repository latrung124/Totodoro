...
PomodoroApiGatewayManager::PomodoroApiGatewayManager(QObject *parent)
    : IApiGatewayManager(parent)
    , m_responseHandler(std::make_shared<JsonResponseHandler>())
{
}

bool PomodoroApiGatewayManager::registerResponseCallback(
    gateway::RequestType requestType, const ResponseCallback &callback)
{
	if (m_responseCallbacks.find(requestType) != m_responseCallbacks.end()) {
		qDebug() << "Callback for request type" << static_cast<int>(requestType)
...
bool PomodoroApiGatewayManager::unregisterResponseCallback(gateway::RequestType requestType)
{
	...
	if (it == m_responseCallbacks.end()) {
		qDebug() << "No callback found for request type" << static_cast<int>(requestType)
		         << "to unregister.";
...
void PomodoroApiGatewayManager::request(const gateway::ApiRequest &request)
{
	qDebug() << "Requesting for request type:" << static_cast<int>(request.requestType);
}

bool PomodoroApiGatewayManager::createPomodoroSession(
    const gateway::PomodoroSessionProperties &sessionProps)
{
	qDebug()
	    << "Creating Pomodoro session for user:" << QString::fromStdString(sessionProps.userId);
	// Sample implementation for creating a Pomodoro session
	QString createSessionBodyQStr = QString::fromStdString(
	    sessionProps.toCreateSessionBodyJsonString());
...
bool PomodoroApiGatewayManager::getPomodoroSessions(
    const std::string &userId, const std::string &taskId)
{
...
bool PomodoroApiGatewayManager::updatePomodoroSession(
    const gateway::PomodoroSessionProperties &sessionProps)
{
	qDebug() << "Updating Pomodoro session:" << QString::fromStdString(sessionProps.sessionId);
	QString updateSessionBodyQStr = QString::fromStdString(
	    sessionProps.toUpdateSessionBodyJsonString());
	ApiCommandFactory &factory = ApiCommandFactory::instance();
