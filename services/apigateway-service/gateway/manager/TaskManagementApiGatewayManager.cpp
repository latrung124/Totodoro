...
bool TaskManagementApiGatewayManager::registerResponseCallback(
...
void TaskManagementApiGatewayManager::request(const gateway::ApiRequest &request)
{
	qDebug() << "Requesting for type:" << static_cast<int>(request.requestType);
}
...
bool TaskManagementApiGatewayManager::createTaskGroup(
    const gateway::TaskGroupProperties &groupProperties)
{
	qDebug() << "Creating task group:" << QString::fromStdString(groupProperties.name);
	QString createGroupBodyQStr = QString::fromStdString(
	    groupProperties.toCreateTaskGroupBodyJsonString());
	ApiCommandFactory &factory = ApiCommandFactory::instance();
...
bool TaskManagementApiGatewayManager::updateTaskGroup(
    const gateway::TaskGroupProperties &groupProperties)
{
	qDebug() << "Updating task group ID:" << QString::fromStdString(groupProperties.groupId);
	QString updateGroupBodyQStr = QString::fromStdString(
	    groupProperties.toUpdateTaskGroupBodyJsonString());
	ApiCommandFactory &factory = ApiCommandFactory::instance();
