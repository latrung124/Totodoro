...
{
	const auto userId = QString::fromStdString(
	    extractPathParam(req, 1)); // 1 is the first capture group
	if (userId.isEmpty()) {
		res.status = 400;
	...
	auto command = std::make_shared<CreateSessionCommand>(
	    userId, body, QString::fromStdString(mBaseUrl));
...
{
	const auto sessionId = QString::fromStdString(
	    extractPathParam(req, 1)); // 1 is the first capture group
	...
	auto command = std::make_shared<GetSessionByIdCommand>(
	    sessionId, QString::fromStdString(mBaseUrl));
...
void SessionRouteHandler::handleUpdateSession(const httplib::Request &req, httplib::Response &res)
{
	...
	auto command = std::make_shared<UpdateSessionCommand>(
	    body, sessionId, QString::fromStdString(mBaseUrl));
	auto responseHandler = createResponseHandler();
	command->setResponseHandler(responseHandler);
...
void SessionRouteHandler::handleDeleteSession(const httplib::Request &req, httplib::Response &res)
{
	...
	auto command = std::make_shared<DeleteSessionCommand>(
	    sessionId, QString::fromStdString(mBaseUrl));
	auto responseHandler = createResponseHandler();
	command->setResponseHandler(responseHandler);
