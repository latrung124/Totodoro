...
bool Gateway::initialize()
{
	...
	mPomodoroController = std::make_shared<PomodoroOpenApiController>(
	    requestProcessor, kBaseUrl.data());

	mApiGatewayManagers[gateway::RouteType::Pomodoro] = ApiGatewayManagerFactory::createManager(
	    gateway::RouteType::Pomodoro);
	mApiGatewayManagers[gateway::RouteType::TaskManagement] =
	    ApiGatewayManagerFactory::createManager(gateway::RouteType::TaskManagement);
	mApiGatewayManagers[gateway::RouteType::User] = ApiGatewayManagerFactory::createManager(
	    gateway::RouteType::User);
