...
OauthCode::OauthCode(QObject *parent)
    : OauthBase(parent)
{
}
...
void OauthCode::link()
{
	connect(
	    &m_server,
	    SIGNAL(dataReceived(QMap<QString, QString>)),
	    this,
...
void OauthCode::authenticationNeededCallback()
{
	QDesktopServices::openUrl(QUrl(
	    m_authUrl.toString() + "?scope=" + m_scope
	    + (m_accessType == "" ? "" : "&access_type=" + m_accessType) + "&response_type=code"
	    + "&state=" + m_state + "&redirect_uri=" + m_redirectUri + "&client_id=" + m_clientId));
	m_server.start();
}
...
OauthImplicit::OauthImplicit(QObject *parent)
    : OauthBase(parent)
{
}
...
void OauthImplicit::setVariables(
    QString authUrl,
    QString scope,
    QString state,
    QString redirectUri,
...
void OauthImplicit::authenticationNeededCallback()
{
	QDesktopServices::openUrl(QUrl(
	    m_authUrl.toString() + "?scope=" + m_scope
	    + (m_accessType == "" ? "" : "&access_type=" + m_accessType) + "&response_type=token"
	    + "&state=" + m_state + "&redirect_uri=" + m_redirectUri + "&client_id=" + m_clientId));
	m_server.start();
}
 ...
 *
 */
OauthCredentials::OauthCredentials(QObject *parent)
    : OauthBase(parent)
{
}
...
void OauthCredentials::setVariables(
    QString tokenUrl, QString scope, QString clientId, QString clientSecret)
{
	m_tokenUrl = QUrl(tokenUrl);
	m_scope = scope;
	m_clientId = clientId;
 ...
 *
 */
OauthPassword::OauthPassword(QObject *parent)
    : OauthBase(parent)
{
}
...
void OauthPassword::setVariables(
    QString tokenUrl,
    QString scope,
    QString clientId,
    QString clientSecret,
...
void ReplyServer::read()
{
	...
	for (auto tokenPair : tokens) {
		QString key = QUrl::fromPercentEncoding(
		    QByteArray().append(tokenPair.first.trimmed().toLatin1()));
		QString value = QUrl::fromPercentEncoding(
		    QByteArray().append(tokenPair.second.trimmed().toLatin1()));
