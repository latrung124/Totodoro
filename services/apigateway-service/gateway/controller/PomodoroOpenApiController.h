 ...
 * Author: trung.la
 * Date: 09-02-2025
 * Description: Header file for PomodoroOpenApiController class that handles Pomodoro-related API
 * endpoints.
 */
...
public:
	...
	PomodoroOpenApiController(AsyncRequestProcessorPtr requestProcessor, const std::string &baseUrl);
...
private:
	 ...
	 *
	 * @param requestProcessor Shared pointer to the AsyncRequestProcessor for handling asynchronous
	 * requests.
	 ...
	 */
	void initializeHandlers(AsyncRequestProcessorPtr requestProcessor, const std::string &baseUrl);
