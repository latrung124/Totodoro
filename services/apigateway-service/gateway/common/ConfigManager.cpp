 ...
 * Author: trung.la
 * Date: 03-27-2025
 * Description: This file contains the definitions for the ConfigManager class, which manages
...
gateway::common::ConfigManager::ConfigManager(std::string_view configFilePath)
{
	...
	for (const auto &service : config["services"]) {
		m_services[service.first.as<std::string>()] =
		    m_env == "cloud"
		        ? service.second["cloud"].as<std::string>()
		        : service.second["local"].as<std::string>();
	}
}
