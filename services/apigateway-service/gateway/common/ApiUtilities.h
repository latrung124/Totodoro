...
struct ApiRequest
{
	...
	std::variant<std::monostate, UserProperties, TaskProperties, PomodoroSessionProperties, std::string>
	    data;
	...
	T getData() const
	{
		...
		if (RequestType::CreateUser == requestType || RequestType::GetUserProperties == requestType
		    || RequestType::UpdateUserProperties == requestType) {
			if (!std::holds_alternative<UserProperties>(data)) {
				throw std::bad_variant_access();
			}
			return std::get<UserProperties>(data);
		} else if (
		    RequestType::CreateTask == requestType || RequestType::GetTasks == requestType
		    || RequestType::UpdateTask == requestType) {
			if (!std::holds_alternative<TaskProperties>(data)) {
				throw std::bad_variant_access();
			}
			return std::get<TaskProperties>(data);
		} else if (
		    RequestType::CreatePomodoroSession == requestType
		    || RequestType::GetPomodoroSessions == requestType
		    || RequestType::UpdatePomodoroSession == requestType) {
