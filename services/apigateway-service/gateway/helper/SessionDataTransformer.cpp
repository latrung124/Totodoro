...
QVariantMap SessionDataTransformer::transformSessionData(const QVariantMap &rawData)
{
	...
	if (transformed.contains("startTime")) {
		transformed["startTimeFormatted"] = TimeTransformer::formatDateTime(
		    transformed["startTime"].toString());
	}

	if (transformed.contains("endTime") && !transformed["endTime"].toString().isEmpty()) {
		transformed["endTimeFormatted"] = TimeTransformer::formatDateTime(
		    transformed["endTime"].toString());
	}

	if (transformed.contains("lastUpdate")) {
		transformed["lastUpdateFormatted"] = TimeTransformer::formatDateTime(
		    transformed["lastUpdate"].toString());
	}
