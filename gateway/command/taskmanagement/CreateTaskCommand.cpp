 ...
 */
 ...
 CreateTaskCommand::CreateTaskCommand(const OAIRequest& body,
 ...
 void CreateTaskCommand::execute()
 {
     if (!mApiClientFactory) {
         qWarning() << "CreateTaskCommand: ApiClientFactory is null";
         if (mResponseHandler) {
     ...
     // Execute request
     mApiClient->taskManagementServiceCreateTask(mBody);
 }
